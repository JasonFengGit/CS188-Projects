...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for dist in livingGhostPositionDistributions:
            mostPossiblePos = max(dist.items(), key = lambda x: x[1])[0] # index 0 is pos, 1 is prob
            dis = self.distancer.getDistance(pacPos, mostPossiblePos)
            if dis < minDis:
                minDis = dis
                tarPos = mostPossiblePos
            
        result = None
        minDis = float("inf")
        for action in legal:
            succPos = Actions.getSuccessor(pacPos, action)
            dis = self.distancer.getDistance(succPos, tarPos)
            if dis < minDis:
                minDis = dis
                result = action
        return result
