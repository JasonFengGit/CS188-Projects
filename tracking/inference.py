...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        "*** YOUR CODE HERE ***"
        total_val = self.total()
        probs = [self[key] / total_val for key in self.keys()]
        sections = [0, probs[0]]
        for i in range(1, len(probs)):
            sections.append(probs[i] + sections[i])
        #print(sections)
        rand = random.random()
        for i in range(1, len(sections)):
            if sections[i-1] <= rand and rand < sections[i]:
                return list(self.keys())[i-1]
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for ghostPos in self.particles:
            dist[ghostPos] += self.getObservationProb(observation, pacPos, ghostPos, jailPos)
        dist.normalize()
        #print(dist)
        if dist.total() == 0:
            self.initializeUniformly(gameState)
    ...
    def elapseTime(self, gameState):
        ...
        for old in self.particles:
            newDist = self.getPositionDistribution(gameState, old)
    ...
    def getBeliefDistribution(self):
        ...
        for each in par_set:
            dist[each] = self.particles.count(each)
        dist.normalize()
        return dist
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for j in range(self.numParticles):
            ...
            for i in range(self.numGhosts):
                ghost = particle[i]
                prob *= self.getObservationProb(observation[i], gameState.getPacmanPosition(), ghost, self.getJailPosition(i))
