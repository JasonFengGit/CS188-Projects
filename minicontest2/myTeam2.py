...
class OffensiveReflexAgent(ReflexCaptureAgent):
  ...
  def getWeights(self, gameState, action):
    return {'successorScore': 100, 'distanceToFood': -1.5, 'dHome': 15, 'ghostDistance':5.5, 'stop': -10000, 'numInvaders':-10000,
...
class DefensiveReflexAgent(ReflexCaptureAgent):
  ...
  def getFeatures(self, gameState, action):
    ...
    if self.de:
      ...
      if len(foodList) > 0: # This should always be True,  but better safe than sorry
        myPos = successor.getAgentState(self.index).getPosition()
        fd = [self.getMazeDistance(myPos, food) for food in foodList]
        minDistance = sorted(fd)[len(fd)//4]
        features['distanceToFood'] = minDistance
  ...
  def getWeights(self, gameState, action):
    return {'numInvaders': -10000, 'onDefense': 15, 'invaderDistance': -35, 'stop': -100, 'reverse': -2,
            'successorScore': 100, 'distanceToFood': -1.5, 'ghostDistance': 6.8}
