...
class OffensiveReflexAgent(ReflexCaptureAgent):
  ...
  def aheadPos(self, a, successor):
    ix, iy = a.getPosition()
    dVector = {"North":(ix, iy-3), "South":(ix, iy+3), "West":(ix-3, iy), "East":(ix+3, iy)}
...
class DefensiveReflexAgent(ReflexCaptureAgent):
  ...
  def getFeatures(self, gameState, action):
    ...
    if len(invaders) > 0:
      ...
      if gameState.getAgentState(self.index).scaredTimer > 5 and min(dists) < 5:
        features['invaderDistance'] = -min(dists)
    if action == Directions.STOP: features['stop'] = 1
    rev = Directions.REVERSE[gameState.getAgentState(self.index).configuration.direction]
  ...
  def getWeights(self, gameState, action):
    return {'numInvaders': -1000, 'onDefense': 15, 'invaderDistance': -35, 'stop': -100, 'reverse': -2}
